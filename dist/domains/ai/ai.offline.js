...
exports.generateOfflinePlan = generateOfflinePlan;
...
function recurrenceFor(theme, taskIndex, weekDays) {
    if (!theme.recurrence || taskIndex !== 0 || weekDays < 5)
        return undefined;
    if (theme.recurrence === 'daily')
        return 'RRULE:FREQ=DAILY;COUNT=3';
 ...
 */
function generateOfflinePlan(input) {
    ...
    for (let w = 0; w < weekCount; w++) {
        ...
        const daysInWeek = w === weekCount - 1 ? durationDays - w * 7 : Math.min(7, durationDays - w * 7);
    ...
    logger_1.logger.info('[AI OFFLINE MODE USED]', {
    ...
    return {
        milestones,
        tasks,
        notes: `Plan generated locally (offline mode) as a ${category} plan for "${goal}" over ${durationDays} days ` +
            `at ~${hoursPerDay}h/day. Adjust tasks to fit your schedule.`,
    };
}
function buildTaskTitle(goal, theme, index) {
