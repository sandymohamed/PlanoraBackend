...
export function buildPlanMessages(input: PlanPromptInput): ChatMessage[] {
  const { maxMilestones } = AI_CONSTANTS;

  const system =
    input.language === "ar"
      ? `أنت مخطط أهداف. أعد JSON فقط بدون شرح أو markdown.
أنشئ حتى ${maxMilestones} مراحل واضحة ومختلفة للهدف.
كل مرحلة يجب أن تحتوي على:
title, target_date, duration_days, description.
ضع أهم الخطوات العملية داخل description كنص قصير.
لا تنشئ tasks.
لا تكرر المراحل.
اجعل المراحل تمثل مراحل حقيقية للتقدم نحو الهدف، وليس أسابيع مكررة.
التواريخ يجب أن تكون متسلسلة داخل مدة الخطة وتنتهي عند الهدف.`
      : `You are a concise goal planner.
Return ONLY JSON. No markdown or commentary.
Create up to ${maxMilestones} distinct milestones.
Each milestone must contain only:
title, target_date, duration_days, description.
Put the key actionable steps inside the description as a short sentence.
Do NOT create tasks.
Do NOT repeat milestones.
Milestones must represent meaningful phases of progress, not repeated weekly activities.
Dates must be sequential and stay within the plan duration, ending at the goal target.`;
