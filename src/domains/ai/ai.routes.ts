...
const calculateMilestoneDate = (
  ...
  if (goalHasTargetDate && goalTargetDate) {
    ...
    const totalDaysAvailable = Math.ceil(
      (goalTargetDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
    );
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const totalDuration = plan.milestones.reduce(
      ...
      for (const milestoneData of plan.milestones) {
        logger.info("AI milestone dates received", {
          goalId: goal.id,
          goalTargetDate: goal.targetDate,
          now,
          milestones: plan.milestones.map((m, index) => ({
            index,
            title: m.title,
        ...
        if (milestoneData.targetDate) {
          ...
          if (isNaN(milestoneDueDate.getTime())) {
            ...
            milestoneDueDate = calculateMilestoneDate(
              now,
              goalTargetDate,
              cumulativeDays,
              totalDuration,
            ...
            if (
              goal.targetDate &&
              goalTargetDate &&
              milestoneDueDate > goalTargetDate
            ) {
              milestoneDueDate = new Date(goalTargetDate);
            }
            // Ensure AI-generated date is not before today
            // If AI returns an invalid/past date, use our deterministic calculation.
            if (milestoneDueDate < now) {
              logger.warn("[AI MILESTONE DATE INVALID]", {
                goalId: goal.id,
                milestoneTitle: milestoneData.title,
                aiTargetDate: milestoneData.targetDate,
                goalTargetDate: goal.targetDate,
              });
      ...
      for (const taskData of plan.tasks) {
        ...
        if (goal.targetDate && goalTargetDate) {
          ...
          const maxTaskOffset = Math.max(
            ...plan.tasks.map((t) => t.dueOffsetDays),
            1,
          );
          const scaledOffset = Math.floor(
            (taskData.dueOffsetDays / maxTaskOffset) * totalDaysAvailable,
          );
      ...
      const milestonesWithTargetDate = createdMilestones.map(
        (milestone: any) => ({
          ...milestone,
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      return res.status(statusCode).json({
        ...
        error: errorMessage,
        message:
          "Failed to generate simple plan. Please try again or contact support if the issue persists.",
      });
    }
  },
);
