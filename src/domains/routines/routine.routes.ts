...
const createRoutineSchema = Joi.object({
  ...
  schedule: Joi.object({
    time: Joi.string()
      .pattern(/^\d{2}:\d{2}$/)
      .optional(), // Expecting UTC time in HH:mm format
...
router.post("/", async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      let routineWithTask;
      try {
        routineWithTask = await routineService.getRoutineById(
          routine.id,
          userId,
        );
      } catch (getError: any) {
        ...
        if (
          getError?.code === "P1017" ||
          getError?.message?.includes("connection")
        ) {
          logger.warn("Connection error when getting routine, retrying...", {
    ...
    if (value.title !== undefined || value.description !== undefined) {
      try {
        ...
        if (routineTasks.length > 0) {
          ...
          await routineService.updateRoutineTask(
            taskToUpdate.id,
            userId,
            updateTaskData,
          );
          ...
          const updatedRoutine = await routineService.getRoutineById(
            routineId,
            userId,
          );
          ...
          if (updatedRoutine && updatedRoutine.enabled) {
            ...
            scheduleRoutineNotifications(routineId, userId).catch((err) =>
              logger.error("Failed to schedule routine notifications:", err),
            );
          } else {
            // Cancel notifications if routine is disabled
            cancelRoutineNotifications(routineId, userId).catch((err) =>
              logger.error("Failed to cancel routine notifications:", err),
            );
          }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const task = await routineService.addTaskToRoutine(
        routineId,
        userId,
        value as CreateRoutineTaskData,
      );
      ...
      if (routine && routine.enabled) {
        ...
        scheduleRoutineTaskNotifications(
          ...
          logger.error("Failed to schedule routine task notification:", err),
        );
      }
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const task = await routineService.updateRoutineTask(
        taskId,
        userId,
        req.body,
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (routine.enabled) {
        scheduleRoutineNotifications(routineId, userId).catch((err) =>
          logger.error(
            "Failed to reschedule routine notifications after reset:",
            err,
          ),
        );
      }
