...
function initializeFirebase(): void {
  ...
  try {
    ...
    logger.debug(
         "11111111111 Initializing Firebase Admin SDK...",
    ...
    // if (serviceAccountPath != undefined && fs.existsSync(serviceAccountPath)) {
    //   console.log("Initializing Firebase Admin SDK with service account path:", serviceAccountPath);
    ...
    // } else
      ...
      logger.debug(
           "222222222222 Initializing Firebase Admin SDK...",
           process.env.FIREBASE_PROJECT_ID,
           process.env.FIREBASE_PRIVATE_KEY,
           process.env.FIREBASE_CLIENT_EMAIL,
         );
      ...
      logger.warn(
        "Firebase not configured — set GOOGLE_APPLICATION_CREDENTIALS or FIREBASE_* env vars",
      );
      return;
    }
...
class PushNotificationService {
  ...
  async getUserPushTokens(userId: string): Promise<UserPushToken[]> {
    ...
    logger.debug(`getUserPushTokens : Retrieved ${pushTokens.length} push tokens for user ${userId}`);
    return pushTokens.filter(
      (t) => new Date(t.registeredAt).getTime() > thirtyDaysAgo,
    );
  }
  ...
  async sendPushNotification(
    ...
    logger.debug("Sending push notification to user:", userId, "with payload:", payload, "checkPreferences:",checkPreferences);
    ...
    if (checkPreferences) {
      ...
      const notifications = ((user?.settings as Record<string, unknown>)
    ...
    const isAlarmTrigger =
      data.notificationType === "ALARM_TRIGGER" || data.type === "alarm";
    ...
    for (const tokenInfo of tokens) {
      try {
        await admin.messaging().send({
          ...
          data: {
            ...data,
            type: data.type || data.notificationType || "notification",
          },
          android: {
            priority: "high",
            notification: {
              channelId,
              sound: isAlarmTrigger
                ? undefined
                : isReminderType
                  ? "alarm"
